...
describe("layoutSankey", () => {
  ...
  it("sizes nodes by max(inflow, outflow) and keeps them inside the canvas", () => {
    const { nodes, height } = layoutSankey(NODES, LINKS, OPTS);
    ...
    expect(height).toBe(OPTS.height);
  ...
  it("gives a tiny node a label-sized bar and grows the canvas to fit", () => {
    const opts = { ...OPTS, minNodeHeight: 18 };
    const { nodes, links, height } = layoutSankey(
      NODES,
      [...LINKS, { source: "acct:checking", target: "cat:coffee", value: 3 }],
      opts,
    );
    const coffee = nodes.find((n) => n.id === "cat:coffee");
    expect(coffee).toBeUndefined(); // not declared as a node
    for (const n of nodes) expect(n.y1 - n.y0).toBeGreaterThanOrEqual(18);
    expect(height).toBeGreaterThanOrEqual(OPTS.height);
    // Ribbons stay value-sized and centred inside an inflated bar.
    const holiday = nodes.find((n) => n.id === "cat:holiday")!;
    const into = links.find((l) => l.target === "cat:holiday")!;
    expect(into.width).toBeLessThanOrEqual(holiday.y1 - holiday.y0 + 0.001);
  });

  it("orders same-source siblings by size, not by float noise", () => {
    // Every sink here hangs off Checking alone, so they share a barycentre —
    // to within an ULP of acc/w. Size has to decide, or the biggest lands at
    // the bottom of the column with its ribbon crossing everything.
    const sinks = [1200, 235.44, 231.01, 190, 142.4];
    const { nodes } = layoutSankey(
      [
        { id: "in:salary", name: "Salary" },
        { id: "acct:checking", name: "Checking" },
        ...sinks.map((v, i) => ({ id: `cat:${i}`, name: `C${i}` })),
      ],
      [
        { source: "in:salary", target: "acct:checking", value: 3000 },
        ...sinks.map((v, i) => ({
          source: "acct:checking",
          target: `cat:${i}`,
          value: v,
        })),
      ],
    ...
    const last = nodes.filter((n) => n.id.startsWith("cat:"));
    expect(last.sort((a, b) => a.y0 - b.y0).map((n) => n.value)).toEqual(sinks);
  });
