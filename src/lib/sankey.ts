...
export interface SankeyLayout {
  nodes: SankeyNode[];
  links: SankeyLink[];
  /** Actual height used — at least `height`, more when minNodeHeight bites. */
  height: number;
}

interface Options {
  width: number;
  /** Target height; the tallest column may push past it (see minNodeHeight). */
  height: number;
  nodeWidth?: number;
  nodePad?: number;
  /**
   * Floor on a node's bar so its label has a row to sit in. A €20 category
   * next to a €3.000 salary is a hairline otherwise, and the labels of a dozen
   * such nodes land on top of each other — which is what made the first
...
export function layoutSankey(
  ...
  if (links.length === 0) return { nodes: [], links: [], height };
  ...
  const barHeight = (n: SankeyNode) => Math.max(minNodeHeight, n.value * k);
  ...
  depths.forEach((d, i) => {
    ...
    if (i === 0) {
      ...
      // Nodes fed by the same single source share a barycentre in theory but
      // differ by an ULP after acc/w, so ties are taken within half a pixel —
      // otherwise float noise beats the size tiebreak and strands a big node
      // at the bottom of the column.
      list.sort((a, b) => {
        const diff = bary.get(a.id)! - bary.get(b.id)!;
        return Math.abs(diff) > 0.5 ? diff : b.value - a.value;
      });
    }
    const columnHeight = stack(list);
    stacked.set(d, columnHeight);
    used = Math.max(used, columnHeight);
    for (const n of list) placed.set(n.id, n);
  });

  // A column inflated by minNodeHeight is taller than a column that wasn't, so
  // the short ones are centred rather than left hanging from the top.
  for (const d of depths) {
    const shift = (used - stacked.get(d)!) / 2;
    if (shift <= 0) continue;
  ...
  // A bar inflated to minNodeHeight is wider than its ribbons, so they're
  // centred in it rather than hung from the top.
  const nodeById = new Map(nodes.map((n) => [n.id, n]));
  const outAt = new Map<string, number>();
  const inAt = new Map<string, number>();
  for (const n of nodes) {
    const h = n.y1 - n.y0;
    outAt.set(n.id, n.y0 + Math.max(0, h - (outflow.get(n.id) ?? 0) * k) / 2);
    inAt.set(n.id, n.y0 + Math.max(0, h - (inflow.get(n.id) ?? 0) * k) / 2);
  }
  ...
  return { nodes, links: out, height: used };
}
