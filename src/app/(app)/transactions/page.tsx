...
import { useTransactions, useDeleteTransaction, useDetectTransfers, useDeleteReimbursement } from "@/hooks/use-transactions";
import type { Transaction, Account, Category, Pot, Pagination } from "@/types/api";
              ...
              <div className="md:hidden rounded-md border divide-y">
                {displayItems.map((item) => {
                  if (item.kind === "pot") {
                    const pot = item.data;
                    return (
                      <div key={`pot-${pot.id}`} className="flex items-center gap-3 px-4 py-3 bg-muted/40 border-t-2">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        {hasReimbursements ? (
                          <>
                            <span className="block text-sm font-mono font-medium text-red-600 dark:text-red-400">
                              {formatCurrency(tx.effectiveAmount)}
                            </span>
                            <span className="block text-xs text-muted-foreground line-through">
                              {formatCurrency(tx.amount)}
                          ...
                          <span
                            className={`text-sm font-mono font-medium ${isInPot ? "line-through " : ""}${
                              isTransfer || isReimbursement || isInPot
                                ? "text-muted-foreground"
                                : tx.amount >= 0
                                  ? "text-emerald-600 dark:text-emerald-400"
