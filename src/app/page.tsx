...
export default async function DashboardPage() {
  ...
  const overallRingColor =
    overallBudgetPct >= 100 ? "#ef4444" : overallBudgetPct >= 80 ? "#f59e0b" : "#3b82f6";
      ...
      {data.budgetItems.length > 0 && (
        <Card className="overflow-hidden">
          <CardContent className="p-0">
            ...
            <div className="flex items-center gap-6 p-6 pb-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  >
                    All Budgets <ArrowRight className="h-3 w-3" />
                  </Link>
                </div>
            ...
            {/* Divider + month progress indicator */}
            <div className="px-6 pb-2">
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                {Math.round(data.monthProgress * 100)}% of the month has passed
              </p>
            </div>

            {/* Category ring chips — horizontal scroll */}
            <div className="px-6 py-4 border-t flex gap-3 overflow-x-auto">
              {data.budgetItems.map((item, i) => {
                ...
                const ringColor =
                  item.status === "exceeded"
                    ...
                    : item.status === "warning"
                      ? "#f59e0b"
                      : item.categoryColor || "#3b82f6";
                const left = Math.max(0, item.limit - item.spent);

                return (
                  <div
                    key={i}
                    className="flex items-center gap-2.5 rounded-xl border px-3 py-2.5 shrink-0 min-w-[160px]"
                  >
                    <div className="relative">
                      <RingProgress
                        percentage={pct}
