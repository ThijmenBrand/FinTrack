...
ALTER TABLE `accounts` ADD `budget_id` text REFERENCES budget_plans(id) ON DELETE set null;--> statement-breakpoint
ALTER TABLE `budgets` ADD `budget_id` text REFERENCES budget_plans(id) ON DELETE cascade;--> statement-breakpoint
...
UPDATE `accounts` SET `budget_id` = (SELECT `id` FROM `budget_plans` WHERE `budget_plans`.`user_id` = `accounts`.`user_id` AND `is_main` = 1) WHERE `type` IN ('checking', 'joint');--> statement-breakpoint
